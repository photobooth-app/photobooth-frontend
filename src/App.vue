...
export default defineComponent({
  ...
  methods: {
    ...
    until(conditionFunction: { (): boolean }) {
      const poll = (resolve) => {
    ...
    initSseClient() {
      ...
      watch(status, (stat: EventSourceStatus) => {
        ...
        this.connected = stat == 'OPEN'
      })

      eventSource.value?.addEventListener('LogRecord', (evt) => {
        this.store.logrecords = [JSON.parse(evt.data), ...this.store.logrecords.slice(0, 199)]
      })
      ...
      eventSource.value?.addEventListener('InformationRecord', (evt) => {
        Object.assign(this.store.information, JSON.parse(evt.data))
      })
    },
